...
Cron-friendly: silent stdout unless threshold changed, errors, or fp>=3.
"""
...
def _parse_serve_log(lines: list[str]) -> dict[str, object]:
    ...
    for line in lines:
        ...
        if "stream_err" in line:
            err += 1
    return {"rtf_avg": _avg(rtf_vals), "err": err}
...
def _write_wake_threshold(settings_path: Path, new_thr: float) -> bool:
    ...
    backup = settings_path.with_name(f"settings.json.bak_{stamp}")
    try:
        shutil.copy2(settings_path, backup)
    except OSError:
        return False
    data["wake_threshold"] = round(new_thr, 2)
    try:
...
def main() -> int:
    ...
    summary = (
        ...
        f"stt_rtf={_fmt_f(serve['rtf_avg'])} err={serve['err']} "
