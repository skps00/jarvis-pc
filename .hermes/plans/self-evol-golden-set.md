...
| 類別 | 目的 | 內容 | 對應測試 |
|---|---|---|---|
| **Golden tasks** | 核心能力不可退化 | 語音喚醒/路由/自我審視/Clarification Gate/autonomy/prompt pipeline/alert/Hands/self-evol/sandbox/Mage-VL video/GPU failover/**STT 準確度 (E2)/resp latency (E3)** 全部核心 module（**唔含 test_eval_gate.py——佢會 nested 重跑本 suite 做成 recursion**） | `test_alert_store.py`、`test_alert_tts_sink.py`、`test_alerts.py`、`test_app_index.py`、`test_autonomy.py`、`test_autostart.py`、`test_brain.py`、`test_brain_translate.py`、`test_clarify.py`、`test_clarify_stats.py`、`test_cursor_hooks.py`、`test_discover.py`、`test_engine.py`、`test_gpu_health.py`、`test_gpu_policy.py`、`test_hands_mc.py`、`test_hermes_bridge.py`、`test_hermes_trusted.py`、`test_mage_engine.py`、`test_prompt_optimizer.py`、`test_prompt_pipeline.py`、`test_router.py`、`test_sandbox.py`、`test_self_review.py`、`test_self_monitor.py`、`test_settings.py`、`test_shell_app.py`、`test_shell_wake_restart.py`、`test_speaker_gate.py`、`test_stt_stats.py`、`test_wake.py` |
