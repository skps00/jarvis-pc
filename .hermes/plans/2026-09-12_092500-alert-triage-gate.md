...
**SK 決定（17:0x）：重新啟動 JARVIS sidecar 嘅時機 = B（全部 task 做完才 restart 一次，我做完會問你）**
...
- [x] 11 Task 7 ✅（`read_miss_ledger()` 24h window＋rotate＋上界＋fail-open；`format_missed_sentence()` 按 id 去重／只計未出聲／純 ASCII／`over 999`；router `alert_miss`（12 句 exact ＋ EN/CJK regex ＋ wake／尾助詞 normalization）；engine 本地 handler（繞 Hermes short-circuit，fail-open 句 `Sir, the alert ledger is unavailable.`）；`tests/test_miss_ledger.py` **19 tests**）
  - 反方 review（read-only subagent）裁決 **REQUEST CHANGES** → 捉到 1 HIGH（句子把 ledger「事件行」當「未答 alert」，已 spoken 都算未答）＋ 2 MEDIUM（phrase 表太窄跌 Hermes；關鍵行為零測試）→ **fix1** 修完；Hermes probe 再捉 2 個 cosmetic（label 消毒次序漏 `>`、`xover 999` 句法）→ **fix2** 修完
  - 最終收貨（2026-09-13 01:0x 親跑）：py_compile OK、targeted **19 passed**、全量 **483 passed / 0 failed**、`--lock` 44 files 一致、三 suite ok、probe（40 項）**全部 PASS**
